...
std::shared_ptr<Node> Storage::get_node(const std::string& path)
{
...
template<typename T>
bool Storage::set_property(const std::string& path, const T& value)
{
...
template<typename T>
bool Storage::get_property(const std::string& path, T& value)
{
   std::string node_path;
   std::string property_name;
   if (!split_property_path(path, node_path, property_name)) {
